...
df = pd.read_excel('./data_chimp/Pant hoots and phases_2.xlsx', dtype={'Build-up ends': float, 'Climax ends': float}, na_values=' ')
...
for filename in list(df['File Name']):
    duration = librosa.get_duration(filename='./data_chimp/{}.wav'.format(filename.replace(';', '_')))
